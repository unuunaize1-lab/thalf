import { createCheckoutSchema } from '../src/validators/order.validator';
...
function runP01CheckoutVerification() {
  ...
  // Test WhatsApp Deep Link formatting
  const mockOrderNumber = 'THF-20260803-9999';
  const whatsappUrl = whatsappService.generateOrderDeepLink({
    orderNumber: mockOrderNumber,
    items: [
      { productName: 'Venezuelan Dark Chocolate Bar 80%', quantity: 2, unitPrice: 1850 },
      { productName: 'The Imperial Gold Gifting Box', quantity: 1, unitPrice: 6500 },
    ],
    totalAmount,
    giftWrap: true,
  ...
  console.log('\n✅ Step 4: WhatsApp Deep Link Generation Verification:');
  console.log(`   - Target Business Phone: +919876500000`);
  console.log(`   - Generated URL: ${whatsappUrl.substring(0, 80)}...`);
