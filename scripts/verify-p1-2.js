...
async function runP12Verification() {
  ...
  assert(svgRes.valid === false && svgRes.error.includes('Security violation'), 'SVG/script injection payload rejected');
  ...
  assert(oversizeRes.valid === false && oversizeRes.error.includes('exceeds maximum allowed threshold'), 'Oversized payload >10MB rejected');

  const exeRes = CloudinaryService.validateImageFile(validBuffer, 'application/octet-stream');
  assert(exeRes.valid === false && exeRes.error.includes('Unsupported image format'), 'Non-image binary extension rejected');
